document.addEventListener("DOMContentLoaded", () => {
  ...
  filter_price();
  document
    .querySelector(".clr-filter-div button")
    .addEventListener("click", reset_filter);
  ...
  if (document.querySelector("#trip-identifier").value === "2") {
    ...
    filter_price2();
    document
      .querySelector(".clr-filter-div2 button")
      .addEventListener("click", reset_filter2);
  }
});
...
function flight_duration() {
  document.querySelectorAll(".flight-stops .tooltiptext").forEach((element) => {
    let time = convertMsToHM(element.dataset.value).split(":");
...
function filter(element = null) {
  ...
  if (true) {
    if (
      document.querySelector(".departure-time-slot-group .square-box.active")
    ) {
      start = document.querySelector(
        ".departure-time-slot-group .square-box.active"
      ).dataset.start;
      end = document.querySelector(
        ".departure-time-slot-group .square-box.active"
      ).dataset.end;
      flights = document.querySelectorAll(
        "#flights_div .each-flight-div-box.show"
      );
    }
  ...
  if (true) {
    if (document.querySelector(".arrival-time-slot-group .square-box.active")) {
      start = document.querySelector(
        ".arrival-time-slot-group .square-box.active"
      ).dataset.start;
      end = document.querySelector(
        ".arrival-time-slot-group .square-box.active"
      ).dataset.end;
...
function filter2(element = null) {
  ...
  if (true) {
    if (
      document.querySelector(".departure-time-slot-group2 .square-box.active")
    ) {
      start = document.querySelector(
        ".departure-time-slot-group2 .square-box.active"
      ).dataset.start;
      end = document.querySelector(
        ".departure-time-slot-group2 .square-box.active"
      ).dataset.end;
      flights = document.querySelectorAll(
        "#flights_div2 .each-flight-div-box.show"
      );
    }
  ...
  if (true) {
    if (
      document.querySelector(".arrival-time-slot-group2 .square-box.active")
    ) {
      start = document.querySelector(
        ".arrival-time-slot-group2 .square-box.active"
      ).dataset.start;
      end = document.querySelector(
        ".arrival-time-slot-group2 .square-box.active"
      ).dataset.end;
...
function arrival_slot(flights, start, end) {
  if (flights) {
    for (let i = 0; i < flights.length; i++) {
      time = flights[i]
        .querySelector(".flight-destination-time .flight-time h5")
        .innerText.split(":");
      if (
        parseInt(time[0]) >= parseInt(start) &&
        parseInt(time[0]) < parseInt(end)
      ) {
...
function departure_slot(flights, start, end) {
  if (flights) {
    for (let i = 0; i < flights.length; i++) {
      time = flights[i]
        .querySelector(".flight-origin-time .flight-time h5")
        .innerText.split(":");
      if (
        parseInt(time[0]) >= parseInt(start) &&
        parseInt(time[0]) < parseInt(end)
...
function arrival_slot2(flights, start, end) {
  if (flights) {
    for (let i = 0; i < flights.length; i++) {
      time = flights[i]
        .querySelector(".flight-destination-time2 .flight-time h5")
        .innerText.split(":");
      if (
        parseInt(time[0]) >= parseInt(start) &&
        parseInt(time[0]) < parseInt(end)
...
function reset_filter() {
  ...
  let max = document
    .querySelector(".filter-price input[type=range]")
    .getAttribute("max");
  ...
  let flights = document
    .querySelector("#flights_div")
    .querySelectorAll(".each-flight-div-box");
  for (let i = 0; i < flights.length; i++) {
    //flights[i].style.display = 'block';
    flights[i].classList.add("show");
    flights[i].classList.remove("hide");
...
function reset_filter2() {
  ...
  let max = document
    .querySelector(".filter-price2 input[type=range]")
    .getAttribute("max");
...
function flight_select() {
  document.querySelectorAll(".flight1-radio").forEach((radio) => {
    radio.addEventListener("click", (e) => {
      ...
      document.querySelector("#select-f1-plane").innerText =
        e.target.dataset.plane;
      document.querySelector("#select-f1-depart").innerText =
        e.target.dataset.depart;
      document.querySelector("#select-f1-arrive").innerText =
        e.target.dataset.arrive;
      document.querySelector("#select-f1-fare").innerText =
        e.target.dataset.fare;
      document.querySelector("#select-total-fare").innerText =
        parseInt(e.target.dataset.fare) +
        parseInt(document.querySelector("#select-f2-fare").innerText);
  ...
  document.querySelectorAll(".flight2-radio").forEach((radio) => {
    radio.addEventListener("click", (e) => {
      ...
      document.querySelector("#select-f2-plane").innerText =
        e.target.dataset.plane;
      document.querySelector("#select-f2-depart").innerText =
        e.target.dataset.depart;
      document.querySelector("#select-f2-arrive").innerText =
        e.target.dataset.arrive;
      document.querySelector("#select-f2-fare").innerText =
        e.target.dataset.fare;
      document.querySelector("#select-total-fare").innerText =
        parseInt(e.target.dataset.fare) +
        parseInt(document.querySelector("#select-f1-fare").innerText);
      document.querySelector("#select-total-fare-media").innerText =
        parseInt(e.target.dataset.fare) +
        parseInt(document.querySelector("#select-f1-fare").innerText);
    });
...
function initial_click() {
  if (window.matchMedia("(max-width: 376px)").matches) {
    if (document.querySelector("#trip-identifier").value === "2") {
      document
        .querySelector(".query-result-div .each-flight-div")
        .classList.add("blue");
      document
        .querySelector(".query-result-div-2 .each-flight-div")
        .classList.add("blue");
    }
  }
}
...
function show_filter() {
  if (Boolean(document.querySelector(".query-result-div-2"))) {
    ...
    document.querySelector(".query-result-div .filter-div").style.display =
      "block";
  }
}
